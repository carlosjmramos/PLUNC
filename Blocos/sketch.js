...
function setup() {
  ...
  // Make the box
  box = new Box(width/2-120,height/2);
  box2 = new Box(width/2-60,height/2);
  box3 = new Box(width/2,height/2);
  box4 = new Box(width/2+60,height/2);
  box5 = new Box(width/2+120,height/2);
...
function mousePressed() {
  // Check to see if the mouse was clicked on the box
	...
	else if (box3.contains(mouseX, mouseY)) {
    // And if so, bind the mouse location to the box with a spring
    spring.bind(mouseX,mouseY,box3);
  	}
	else if (box4.contains(mouseX, mouseY)) {
    // And if so, bind the mouse location to the box with a spring
    spring.bind(mouseX,mouseY,box4);
  	}
  	else if (box5.contains(mouseX, mouseY)) {
    // And if so, bind the mouse location to the box with a spring
    spring.bind(mouseX,mouseY,box5);
  	}
}